...
| `take_camera` | optional, free text, one move for the WHOLE shot ("a slow, steady clockwise arc around the table at eye level, wide to close"). `take_camera_lock()` puts it first in part 1's prompt ("Camera: …; one steady move at one speed, never stops, never changes direction") and, in every later part, a sentence that the camera is *already* moving that way and continues in the same direction at the same speed — the handoff anchors the picture between parts, not the motion, and the seam was visible as a camera that changed direction or stopped and restarted at every join. Left blank, later parts still get the generic continuity sentence. Recorded as `take.camera` |
| `take_handoff` | `last` (default) or `speech`. With `speech` the runner cuts every part but the last on a TALKING frame: `take_handoff_points()` finds where the line ends (audio RMS), stops the picture `TAKE_TALKING_BACK` = 0.2 s before it (the mouth still mid-word — the only anchor measured to carry the voice onto the next part's mouth: +0.54 / +0.47 against −0.15…−0.01 on a closed mouth), keeps the sound to the end of the line plus its 0.4 s decay, and mixes that tail (`part{k}_tail.wav`, `amix normalize=0`) over the head of the next part (`part{k+1}_lead.mp4`) — a J-cut, so the last word is heard whole. Write each part's first beat to OPEN IN SILENCE so the tail has room. The silent tail after each line is dropped, so the take runs shorter than its nominal length; the last part is not trimmed. Every spoken part is scored by `take_lipsync_score()` and retaken up to `TAKE_LIPSYNC_RETAKES` = 2 times with fresh seeds (ids `-pNl`, `-pNl2`) when under `TAKE_LIPSYNC_MIN` = 0.20; the best clip is kept and its score recorded in `job["take_lipsync"]`. Recorded as `take.handoff` |
